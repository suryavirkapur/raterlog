...
# PostgreSQL, Cassandra, MailHog, the Rust API, and the web server.
set -euo pipefail
...
gosu postgres psql -q -d example -f /app/web/schema.sql
log "PostgreSQL ready on :5432 (schema applied)"
