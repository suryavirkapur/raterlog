...
import { createToken, deleteToken, getCompanyTokens } from "@/server/data";
import { TabBar } from "@/components/TabBar";
import { TokenCard } from "@/components/TokenCard";

export const Route = createFileRoute("/dash/$companyId/tokens")({
  loader: async ({ params }) =>
    await getCompanyTokens({ data: { companyId: params.companyId } }),
  component: TokensTab,
});
...
function TokensTab() {
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await createToken({
      data: { companyId, name: String(fd.get("name") || "") },
    });
    setPending(false);
  ...
  async function onDelete(tokenId: number) {
