...
export function ChannelAiPanel({ channelId }: { channelId: string }) {
  ...
  async function summarize() {
    ...
    setError("");
    try {
      setSummary(await api<AiSummary>(`/api/channels/${channelId}/ai/summarize`, { method: "POST", body: "{}" }));
    } catch (err) {
    ...
    <Card size="2">
      <Flex direction="column" gap="3">
        ...
        <Text size="2" color="gray">
          Summarize this channel or ask a question about recent events. Heuristics
          always run; an LLM is used when OPENAI_API_KEY is set on the API.
        </Text>
        <Flex gap="2">
